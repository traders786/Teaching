...
export function TeacherLayout({
  ...
  const navItems = [
    { id: 'dashboard' as TeacherTab, label: 'Dashboard', icon: LayoutDashboard, badge: null },
    { id: 'profile' as TeacherTab, label: 'My Profile', icon: UserCheck, badge: null },
    { id: 'demos' as TeacherTab, label: 'Demo Students', icon: Sparkles, badge: 'Live' },
    { id: 'classes' as TeacherTab, label: 'Class Management', icon: Users, badge: null },
    { id: 'curriculum' as TeacherTab, label: 'Curriculum Board', icon: BookOpen, badge: null },
    { id: 'homework' as TeacherTab, label: 'Homework', icon: FileText, badge: null },
    { id: 'recordings' as TeacherTab, label: 'Recorded Sessions', icon: Video, badge: null },
    { id: 'helpdesk' as TeacherTab, label: 'Helpdesk', icon: LifeBuoy, badge: null },
  ];
    ...
    <div className="min-h-screen bg-[#FAF9F6] flex flex-col font-sans text-slate-800">
      ...
      <div className="bg-[#10182C] text-slate-300 text-xs px-4 py-2 flex flex-wrap items-center justify-between border-b border-slate-800">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 bg-[#F27C00]/20 text-[#F27C00] font-semibold px-2 py-0.5 rounded text-[11px]">
            <GraduationCap className="w-3.5 h-3.5" />
            TEACHER PORTAL
          </span>
          <span className="hidden sm:inline text-slate-400">|</span>
          <span className="text-slate-300">Logged in as: <strong className="text-white font-medium">{user.name}</strong> (Faculty Coach)</span>
        </div>
          ...
          {onSwitchRole && (
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="px-2 py-0.5 rounded bg-[#F27C00] text-white font-medium text-[11px]"
              >
                Teacher
              </button>
              <button
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline-block text-[11px] font-semibold uppercase tracking-wider text-slate-600 bg-slate-100 px-2 py-0.5 rounded">
                Educator Hub
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 pl-3 border-l border-slate-200">
              {user?.avatar_url ? (
                ...
                <div className="w-8 h-8 rounded-full bg-[#10182C] text-white flex items-center justify-center font-bold text-xs">
                  {user.name ? user.name[0] : 'T'}
                </div>
              )}
              <div className="hidden md:block text-left text-xs leading-tight">
                <div className="font-semibold text-[#10182C]">{user.name}</div>
                <div className="text-slate-600">Senior Faculty Coach</div>
              </div>
              <button
                onClick={onLogout}
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge && (
                    <span className="text-[10px] bg-[#F27C00] text-white px-1.5 py-0.5 rounded-full font-bold">
                      {item.badge}
                    </span>
                  )}
                </button>
            ...
            <div className="relative w-72 max-w-xs bg-white h-full shadow-2xl p-4 flex flex-col z-50">
              <div className="flex items-center justify-between pb-4 border-b border-slate-200">
                <span className="font-bold text-lg text-[#10182C]">
                  up<span className="text-[#F27C00]">speaq</span> Educator
                </span>
